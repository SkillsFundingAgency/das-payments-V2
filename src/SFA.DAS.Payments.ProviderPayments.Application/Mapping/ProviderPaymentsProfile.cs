    ...
    public class ProviderPaymentsProfile : Profile
    {
        public ProviderPaymentsProfile()
        {
            ...
            CreateMap<FundingSourcePaymentEvent, PaymentModel>()
                ...
                .Include<TransferFundingSourcePaymentEvent, PaymentModel>()
                .ForMember(dest => dest.EventId, opt => opt.ResolveUsing(src => Guid.NewGuid()))
                .ForMember(dest => dest.FundingSourceEventId, opt => opt.MapFrom(source => source.EventId))
                ...
                .ForMember(dest => dest.StartDate, opt => opt.ResolveUsing(source => source.StartDate == DateTime.MinValue ? DateTime.Today: source.StartDate)) //TODO: review why pv2-1025 used DateTime.Today
                .ForMember(dest => dest.PlannedEndDate, opt => opt.ResolveUsing(source => source.PlannedEndDate.HasValue && source.PlannedEndDate == DateTime.MinValue ? null : source.PlannedEndDate))
                .ForMember(dest => dest.ActualEndDate, opt => opt.ResolveUsing(source => source.ActualEndDate.HasValue && source.ActualEndDate == DateTime.MinValue ? null : source.ActualEndDate))
            ...
            CreateMap<TransferFundingSourcePaymentEvent, PaymentModel>();
