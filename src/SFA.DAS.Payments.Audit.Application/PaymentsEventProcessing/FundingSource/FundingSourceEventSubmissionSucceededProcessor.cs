...
namespace SFA.DAS.Payments.Audit.Application.PaymentsEventProcessing.FundingSource
{
    ...
    public class FundingSourceEventSubmissionSucceededProcessor : IFundingSourceEventSubmissionSucceededProcessor
    {
        ...
        public async Task Process(SubmissionJobSucceeded message, CancellationToken cancellationToken)
        {
            ...
            logger.LogDebug($"Flushed cache. Now removing old funding source events for job: {message.JobId}, provider: {message.Ukprn}, collection period: {message.CollectionPeriod}");
            ...
            logger.LogInfo($"Finished removing old funding source events for job: {message.JobId}, provider: {message.Ukprn}, collection period: {message.CollectionPeriod}");
        }
    }
}