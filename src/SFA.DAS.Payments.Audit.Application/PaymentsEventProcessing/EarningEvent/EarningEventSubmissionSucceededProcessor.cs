...
namespace SFA.DAS.Payments.Audit.Application.PaymentsEventProcessing.EarningEvent
{
    ...
    public class EarningEventSubmissionSucceededProcessor: IEarningEventSubmissionSucceededProcessor
    {
        ...
        {
            logger.LogVerbose($"Flushing cached earning events before removing old data for provider. Job: {message.JobId}, collection period: {message.CollectionPeriod}");
            await batchService.StorePayments(cancellationToken).ConfigureAwait(false);
            logger.LogDebug($"Flushed cache. Now removing old earning events for provider. Job: {message.JobId}, collection period: {message.CollectionPeriod}");
            ...
            logger.LogInfo($"Finished removing old earning events for provider. Job: {message.JobId}, collection period: {message.CollectionPeriod}");
        }
    }
}