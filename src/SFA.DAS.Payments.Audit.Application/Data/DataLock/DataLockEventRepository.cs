    ...
    public class DataLockEventRepository : IDataLockEventRepository
    {
        ...
        public async Task SaveDataLockEvents(List<DataLockEventModel> dataLockEvents, CancellationToken cancellationToken)
        {
            using (var tx = await dataContext.Database.BeginTransactionAsync(IsolationLevel.ReadUncommitted, cancellationToken).ConfigureAwait(false))
            {
                ...
                await ((DbContext)dataContext).BulkInsertAsync(dataLockEvents.SelectMany(earning => earning.PriceEpisodes).ToList(), bulkConfig, null, cancellationToken)
                    .ConfigureAwait(false);
                await ((DbContext)dataContext).BulkInsertAsync(dataLockEvents.SelectMany(earning => earning.PayablePeriods).ToList(), bulkConfig, null, cancellationToken)
                    .ConfigureAwait(false);
                await ((DbContext)dataContext).BulkInsertAsync(dataLockEvents.SelectMany(earning => earning.NonPayablePeriods).ToList(), bulkConfig, null, cancellationToken)
                    .ConfigureAwait(false);
