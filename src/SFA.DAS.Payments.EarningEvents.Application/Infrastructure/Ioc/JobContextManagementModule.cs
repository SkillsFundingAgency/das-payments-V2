    ...
    public class JobContextManagementModule : Module
    {
        protected override void Load(ContainerBuilder builder)
        {
            ...
            builder.Register(c =>
            {
                ...
                var topicSubscriptionConfig = new TopicConfiguration(configHelper.GetConnectionString("ServiceBusConnectionString"), configHelper.GetSetting("TopicName"), configHelper.GetSetting("SubscriptionName"), 1, maximumCallbackTimeSpan: TimeSpan.FromMinutes(40));
            ...
            {
                var configHelper = c.Resolve<IConfigurationHelper>();
                var auditPublishConfig = new QueueConfiguration(configHelper.GetConnectionString("ServiceBusConnectionString"), configHelper.GetSetting("AuditQueueName"), 1);
            ...
            {
                var configHelper = c.Resolve<IConfigurationHelper>();
                var jobStatusPublishConfig = new QueueConfiguration(configHelper.GetConnectionString("ServiceBusConnectionString"), configHelper.GetSetting("JobStatusQueueName"), 1);
