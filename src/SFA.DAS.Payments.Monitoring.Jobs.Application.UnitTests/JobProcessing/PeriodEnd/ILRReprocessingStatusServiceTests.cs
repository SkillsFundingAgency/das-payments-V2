...
{
    [TestFixture]
    public class ILRReprocessingStatusServiceTests
    {
        ...
        [Test]
        public async Task TimedOutJobs_CompletedWithErrors()
        {
            var jobId = 99;
            ...
            mocker.Mock<IJobStorageService>()
                .Verify(
                    x => x.SaveJobStatus(It.Is<long>(id => id == job.DcJobId.Value),
        ...
        [Test]
        public async Task Continues_Until_All_Jobs_Complete()
        {
            var jobId = 99;
        ...
        [Test]
        public async Task Uses_UtcNow_As_JobEndTime()
        {
            var jobId = 99;
