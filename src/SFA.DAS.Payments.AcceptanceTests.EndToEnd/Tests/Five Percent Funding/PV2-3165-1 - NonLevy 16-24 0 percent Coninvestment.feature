Feature: PV2-3165-1 - NonLevy 16-24 0 percent Coninvestment
As a provider,
...
Scenario Outline: Training for a learner starting in January 2023 with an On-Service NonLevy employer is fully funded by the Government 
	Given the apprentice is employed by an On-Service employer who has never declared Levy
	And the apprentice is doing an Apprenticeship which starts in January 2023
	And the apprentice is aged between 16 and 24 when the apprenticeship starts
	When the provider submits the training details for the apprenticeship learning
	Then the earnings generated by the earnings calc should have generated a default co-investment rate of 5% 
