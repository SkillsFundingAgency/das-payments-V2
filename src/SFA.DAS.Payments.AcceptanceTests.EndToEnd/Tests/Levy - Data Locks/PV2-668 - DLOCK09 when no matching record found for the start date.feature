...
Scenario Outline: DLOCK09 - When no matching record found in an employer digital account for the start date then datalock DLOCK_09 will be produced_PV2-668
    Given the employer levy account balance in collection period <Collection_Period> is 11000
    And the following commitments exist
        | Identifier       | Framework code | Programme type | Pathway code | start date                   | end date                  | status | agreed price | effective from               | effective to |
        | Apprenticeship a | 593            | 20             | 1            | 01/Sep/Current Academic Year | 01/Oct/Next Academic Year | active | 10000        | 01/Sep/Current Academic Year |              |
