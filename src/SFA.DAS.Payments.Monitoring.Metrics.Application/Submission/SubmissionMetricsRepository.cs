...
namespace SFA.DAS.Payments.Monitoring.Metrics.Application.Submission
{
    ...
    public class SubmissionMetricsRepository : ISubmissionMetricsRepository
    {
        ...
        {
            var submissions = await persistenceDataContext.SubmissionSummaries.Include(s => s.DataLockMetrics).Where(s => s.CollectionPeriod == currentCollectionPeriod && s.AcademicYear == academicYear).ToListAsync(cancellationToken: cancellationToken);
