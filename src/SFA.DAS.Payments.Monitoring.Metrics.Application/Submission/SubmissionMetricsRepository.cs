    ...
    public class SubmissionMetricsRepository : ISubmissionMetricsRepository
    {
        ...
        public async Task<List<TransactionTypeAmounts>> GetRequiredPayments(long ukprn, long jobId, CancellationToken cancellationToken)
        {
            using (await QueryDataContext.BeginTransaction(cancellationToken))
            {
                ...
                var fundingSouce = new List<FundingSourceType> { FundingSourceType.CoInvestedEmployer, FundingSourceType.CoInvestedSfa, FundingSourceType.FullyFundedSfa };
                ...
                var clawBackTransactionAmounts = await QueryDataContext.Payments
                    ...
                    .Where(rp => rp.Ukprn == ukprn && 
                                 ...
                                 fundingSouce.Contains(rp.FundingSource))
                    .GroupBy(rp => new { rp.ContractType, rp.TransactionType })
                    .Select(group => new
                    {
                        ContractType = group.Key.ContractType,
