...
{
    public interface IPeriodEndJobIlrReprocessingStatusService : IJobStatusService
    {
    }

    public class PeriodEndJobIlrReprocessingStatusService : JobStatusService, IPeriodEndJobIlrReprocessingStatusService
    {
        private readonly IJobsDataContext context;

        public PeriodEndJobIlrReprocessingStatusService(IJobStorageService jobStorageService, IPaymentLogger logger,
        ...
        public override async Task<(bool IsComplete, JobStatus? OverriddenJobStatus, DateTimeOffset? completionTime)>
            PerformAdditionalJobChecks(JobModel job, CancellationToken cancellationToken)
        {
            ...
            if (processingJobsPresent.Any())
            {
                ...
                var completionTimesForInProgressJobs =
                    await context.GetAverageJobCompletionTimesForInProgressJobs(
            ...
            {
                Logger.LogDebug(
                    $"{jobsWithoutSubmissionSummariesPresent.Count} File Processing jobs without Submission Summaries Present during Period End Start job, Period End Start JobId {job.DcJobId}");
            ...
            Logger.LogDebug(
                $"No Outstanding Jobs or jobs Without Submission Summaries during Period End Start job, Now updating Period End Start job status to Completed, Period End Start JobId {job.DcJobId}");
            ...
            CancellationToken cancellationToken)
        {
            foreach (var inProgressJob in processingJobs)
            {
                ...
                if (inProgressJob.JobRunTimeDurationInMillisecond > (providerJobTimings.AverageJobCompletionTime ?? 0))
                {
                    ...
                    await context.SaveJobStatus(inProgressJob.DcJobId.Value, JobStatus.TimedOut, DateTimeOffset.UtcNow,
                        cancellationToken);
                }
                else
                {
        ...
        protected override async Task<bool> IsJobTimedOut(JobModel job, CancellationToken cancellationToken)
        {
            ...
            if (isJobTimedOut) await EventPublisher.PeriodEndJobFinished(job, false);
