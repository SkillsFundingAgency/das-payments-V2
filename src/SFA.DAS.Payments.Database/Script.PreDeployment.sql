...
--or the main table scripts which also include the indexes 				
			SELECT *
			FROM sys.columns
			WHERE name = N'DuplicateNumber'
				AND object_id = OBJECT_ID(N'Payments2.DataLockEvent')
			)
BEGIN
	BEGIN TRAN
	...
	IF NOT EXISTS (
			SELECT *
			FROM sys.indexes
			WHERE name = N'UX_DataLockEvent_LogicalDuplicates'
				AND object_id = OBJECT_ID(N'Payments2.DataLockEvent')
			)
	...
	IF NOT EXISTS (
			...
			FROM sys.columns
			WHERE name = N'DuplicateNumber'
				AND object_id = OBJECT_ID(N'Payments2.FundingSourceEvent')
			)
	...
	IF NOT EXISTS (
			SELECT *
			FROM sys.indexes
			WHERE name = N'UX_FundingSourceEvent_LogicalDuplicates'
				AND object_id = OBJECT_ID(N'Payments2.FundingSourceEvent')
			)
	...
	IF NOT EXISTS (
			...
			FROM sys.columns
			WHERE name = N'DuplicateNumber'
				AND object_id = OBJECT_ID(N'Payments2.Payment')
			)
	...
	IF NOT EXISTS (
			SELECT *
			FROM sys.indexes
			WHERE name = N'UX_Payment_LogicalDuplicates'
				AND object_id = OBJECT_ID(N'Payments2.Payment')
			)
	...
	IF NOT EXISTS (
			SELECT *
			FROM sys.indexes
			WHERE name = N'UX_RequiredPaymentEvent_LogicalDuplicates'
				AND object_id = OBJECT_ID(N'Payments2.RequiredPaymentEvent')
			)
